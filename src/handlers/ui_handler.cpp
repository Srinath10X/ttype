...
void UiHandler::paddingLeft(unsigned left_padding) {
  std::cout << std::string(left_padding, ' ');
}

void UiHandler::paddingTop(unsigned top_padding) {
  std::cout << std::string(top_padding, '\n');
}

void UiHandler::paddingTopWithOffset(unsigned offset) {
  std::cout << std::string((window.ws_row - offset) / 2, '\n');
}

void UiHandler::paddingLeftWithOffset(unsigned offset) {
  std::cout << std::string((window.ws_col - offset) / 2, ' ');
}
